        ...
        <div className="max-w-6xl mx-auto p-4">
            {loading ? (
                ...
                <>
                    <div className="mb-4 bg-white rounded-lg shadow-md p-4 border-l-4 border-amber-500">
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex-1">
                                <h1 className="text-2xl font-bold text-slate-900 mb-1.5">{caseData.title}</h1>
                    ...
                    <div className="mb-4 bg-white rounded-lg shadow-sm border-b border-slate-200">
                        <nav className="flex space-x-4 px-4" aria-label="Tabs">
                            {tabs.map((tab, idx) => (
                                <button
                                    key={tab}
                                    className={`py-2.5 font-semibold border-b-2 transition-colors duration-150 text-sm ${activeTab === idx
...
const CaseDocumentsTab: React.FC<{ caseId: string }> = ({ caseId }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-4">
            <form onSubmit={handleUpload} className="flex items-center gap-2 mb-4 pb-3 border-b border-slate-200">
                <input
                    type="file"
                    name="docfile"
                    accept="image/*,.pdf,.doc,.docx"
                    className="flex-1 rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent"
                    required
                />
                <button
                    type="submit"
                    disabled={uploading}
                    className="bg-slate-900 text-white rounded px-4 py-2 hover:bg-slate-800 transition font-semibold shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
                >
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-4">
            <div className="flex justify-between items-center mb-3 pb-2 border-b border-slate-200">
                <h2 className="font-bold text-lg text-slate-900">Hearings</h2>
                <button
                    className="bg-slate-900 text-white rounded px-3 py-1.5 hover:bg-slate-800 transition font-semibold shadow-md flex items-center gap-2 text-sm"
                    onClick={() => openModal()}
                >
            ...
            {loading ? (
                ...
                <ul className="space-y-2">
                    {hearings.map(h => (
                        <li key={h.id} className="py-2.5 px-3 rounded border border-slate-200 bg-slate-50 hover:bg-slate-100 transition flex justify-between items-center gap-3">
                            <div className="flex-1 min-w-0">
                                <div className="font-semibold text-slate-900 text-sm mb-0.5">{h.title}</div>
                                <div className="text-xs text-slate-600 flex items-center gap-1.5">
                                    <FaCalendarAlt className="text-amber-600 shrink-0" /> <span>{h.date}</span> {h.notes && <> - <span className="italic truncate">{h.notes}</span></>}
                                </div>
                            </div>
                            <div className="flex gap-1.5 shrink-0">
                                <button onClick={() => openModal(h)} className="px-2.5 py-1 bg-amber-600 text-white rounded hover:bg-amber-700 transition font-medium text-xs flex items-center gap-1">
                                    <FaEdit /> Edit
                                </button>
                                <button onClick={() => handleDelete(h.id)} className="px-2.5 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition font-medium text-xs flex items-center gap-1">
                                    <FaTrash /> Del
                                </button>
                            </div>
                        </li>
            ...
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
                    <form
                        ...
                        className="bg-white rounded-lg p-8 shadow-2xl space-y-4 w-full max-w-md border-t-4 border-amber-500"
                    >
                        <h2 className="text-xl font-bold text-slate-900 mb-4">{editHearing ? "Edit Hearing" : "Add Hearing"}</h2>
                        <div>
                        ...
                        <div className="flex gap-2 pt-4">
                            <button type="submit" className="flex-1 bg-slate-900 text-white rounded-lg px-4 py-3 hover:bg-slate-800 transition font-semibold">{editHearing ? "Update" : "Add Hearing"}</button>
                            <button type="button" className="flex-1 bg-slate-200 text-slate-700 px-4 py-3 rounded-lg hover:bg-slate-300 transition font-semibold" onClick={() => { setShowModal(false); setEditHearing(null); }}>Cancel</button>
                        </div>
                    </form>
                </div>
            )}
...
const CaseTasksTab: React.FC<{ caseId: string }> = ({ caseId }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-4">
            <div className="flex justify-between items-center mb-3 pb-2 border-b border-slate-200">
                <h2 className="font-bold text-lg text-slate-900">Tasks</h2>
                <button
                    className="bg-slate-900 text-white rounded px-3 py-1.5 hover:bg-slate-800 transition font-semibold shadow-md flex items-center gap-2 text-sm"
                    onClick={() => openModal()}
                >
            ...
            {loading ? (
                ...
                <ul className="space-y-2">
                    {tasks.map(t => (
                        <li key={t.id} className="py-2.5 px-3 rounded border border-slate-200 bg-slate-50 hover:bg-slate-100 transition flex justify-between items-center gap-3">
                            <div className="flex-1 min-w-0">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    ...
                                    />
                                    <span className={`text-sm ${t.completed ? "line-through text-slate-400" : "text-slate-900"}`}>{t.text}</span>
                                </label>
                                {t.notes && <div className="text-xs text-slate-500 mt-1 ml-6 truncate">{t.notes}</div>}
                            </div>
                            <div className="flex gap-1.5 shrink-0">
                                <button onClick={() => openModal(t)} className="px-2.5 py-1 bg-amber-600 text-white rounded hover:bg-amber-700 transition font-medium text-xs flex items-center gap-1">
                                    <FaEdit /> Edit
                                </button>
                                <button onClick={() => handleDelete(t.id)} className="px-2.5 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition font-medium text-xs flex items-center gap-1">
                                    <FaTrash /> Del
                                </button>
                            </div>
                        </li>
            ...
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
                    <form
                        ...
                        className="bg-white rounded-lg p-8 shadow-2xl space-y-4 w-full max-w-md border-t-4 border-amber-500"
                    >
                        <h2 className="text-xl font-bold text-slate-900 mb-4">{editTask ? "Edit Task" : "Add Task"}</h2>
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-1">Task</label>
                            <input
                                {...register("text")}
                                className="mt-1 w-full rounded-lg border border-slate-300 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent"
                                required
                        ...
                        <div className="flex items-center gap-2">
                            <input type="checkbox" {...register("completed")} id="completed" className="w-5 h-5 text-amber-600 rounded focus:ring-2 focus:ring-amber-500" />
                            <label htmlFor="completed" className="text-sm font-semibold text-slate-700">Completed?</label>
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-1">Notes (optional)</label>
                            <textarea
                        ...
                        <div className="flex gap-2 pt-4">
                            <button type="submit" className="flex-1 bg-slate-900 text-white rounded-lg px-4 py-3 hover:bg-slate-800 transition font-semibold">{editTask ? "Update" : "Add Task"}</button>
                            <button type="button" className="flex-1 bg-slate-200 text-slate-700 px-4 py-3 rounded-lg hover:bg-slate-300 transition font-semibold" onClick={() => { setShowModal(false); setEditTask(null); }}>Cancel</button>
                        </div>
                    </form>
                </div>
            )}
        ...
        <div className="bg-white rounded-lg shadow-md p-4 flex flex-col h-[500px]">
            <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-200">
                <h2 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                    <FaComments className="text-amber-600" /> Conversations
                </h2>
            </div>
